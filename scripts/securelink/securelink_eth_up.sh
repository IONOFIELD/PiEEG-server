#!/usr/bin/env bash
# securelink_eth_up.sh — configure the direct Ethernet link for secure-link mode.
#
# WHAT IT DOES
#   Creates (once) and enables a NetworkManager profile named
#   "pieeg-secure-link-eth" that gives eth0 the fixed secure-link address 192.168.77.1/24
...
#   is the exact address securelink_stream.py looks for to enter Ethernet mode.
#
...
#   the secure link IP, never a DHCP attempt.
#
# WHAT IT DOES NOT DO
#   It does NOT touch Wi-Fi. Wi-Fi is only brought down by the secure-link streamer
#   itself, after it has successfully bound to this Ethernet IP — so mode
#   switching stays visible, never silent.
#
# USAGE
#   ./securelink_eth_up.sh          (run as your normal user)
# UNDO
#   ./securelink_eth_down.sh        (stop it activating; --delete removes it fully)
set -euo pipefail

IFACE="eth0"
CON_NAME="pieeg-secure-link-eth"
SECURELINK_IP="192.168.77.1/24"

# Create the profile only if it doesn't already exist.
if ! nmcli -t -f NAME connection show | grep -qx "${CON_NAME}"; then
  echo "Creating NetworkManager profile ${CON_NAME} (${SECURELINK_IP} on ${IFACE}, no gateway)..."
  nmcli connection add type ethernet ifname "${IFACE}" con-name "${CON_NAME}" \
    ipv4.method manual ipv4.addresses "${SECURELINK_IP}" \
...
if [ "$(cat /sys/class/net/${IFACE}/carrier 2>/dev/null || echo 0)" = "1" ]; then
  ...
  echo "No cable detected right now. That is fine: the secure link IP will come up"
...
echo
echo "Next step: plug in the laptop, then run ./securelink_preflight.sh to verify."

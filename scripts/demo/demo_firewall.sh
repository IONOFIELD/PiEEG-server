#!/usr/bin/env bash
# demo_firewall.sh — restrict the demo stream port to the laptop's IP only.
#
# WHAT IT DOES
#   Adds a dedicated nftables table ("pieeg_demo") with one job: on the demo
...
#   sudo ./demo_firewall.sh apply 192.168.77.2   # Ethernet demo laptop
#   sudo ./demo_firewall.sh apply 10.0.0.23      # or the laptop's Wi-Fi IP
#   sudo ./demo_firewall.sh status
#   sudo ./demo_firewall.sh remove               # after the demo
set -euo pipefail
...
case "${1:-}" in
  apply)
    ...
    nft "add chain inet ${TABLE} demo_in { type filter hook input priority -5; policy accept; }"
    nft "add rule inet ${TABLE} demo_in tcp dport ${PORT} ip saddr ${LAPTOP_IP} accept"
    nft "add rule inet ${TABLE} demo_in tcp dport ${PORT} drop"
    echo "Applied: port ${PORT} now reachable ONLY from ${LAPTOP_IP}."
    nft list table inet ${TABLE}
    ;;
  status)
    nft list table inet ${TABLE} 2>/dev/null \
      || echo "No pieeg demo firewall rules are active."
    ;;
  remove)
    if nft list table inet ${TABLE} >/dev/null 2>&1; then
      nft delete table inet ${TABLE}
      echo "Removed the pieeg demo firewall table (port ${PORT} back to default policy)."
    else
      echo "Nothing to remove — no pieeg demo firewall rules are active."
    fi
    ;;
  *)
    usage

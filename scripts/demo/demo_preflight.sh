#!/usr/bin/env bash
# demo_preflight.sh — one command to answer: "is the demo link ready?"
#
# Checks, in order:
#   1. cable plugged in (carrier on eth0)
#   2. demo IP 192.168.77.1 present on eth0
...
echo "=== PiEEG demo link preflight ==="
...
# 2. demo IP — did the pieeg-demo-eth profile come up?
if ip -4 addr show dev "${IFACE}" 2>/dev/null | grep -q "inet ${DEMO_IP}/"; then
  ok "demo IP ${DEMO_IP} is on ${IFACE}"
else
  bad "demo IP ${DEMO_IP} is NOT on ${IFACE}" \
      "run ./demo_eth_up.sh (then re-plug the cable if needed)"
fi

# 3. certificate — exists, and lists the demo IP so the laptop will trust it.
if [ -f "${CERT}" ]; then
  if openssl x509 -in "${CERT}" -noout -ext subjectAltName 2>/dev/null \
       | grep -q "${DEMO_IP}"; then
    EXPIRY="$(openssl x509 -in "${CERT}" -noout -enddate | cut -d= -f2)"
    ok "certificate covers ${DEMO_IP} (valid until ${EXPIRY})"
  else
    bad "certificate exists but does NOT list ${DEMO_IP}" \
        "re-run ./gen_demo_cert.sh and copy the new demo-cert.pem to the laptop"
  fi
else
  bad "no certificate at ${CERT}" "run ./gen_demo_cert.sh"
fi
...
echo "  connect to : wss://${DEMO_IP}:${PORT}"
echo "  laptop's own wired IP must be: 192.168.77.2  (mask 255.255.255.0, no gateway)"
echo "  trust file : demo-cert.pem  — copy it over BEFORE the demo:"
...
if [ "${FAIL}" -eq 0 ]; then
  ...
  echo "  cd ${REPO_ROOT} && .venv/bin/python -m pieeg_server.demo_stream"
  exit 0
else
